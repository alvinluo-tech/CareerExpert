...
## 追加(2026-09-02):对抗性实测(3 个零记忆 agent)→ 第二轮重构

### 实测设计
三个互不知情的子 agent 仅凭技能文件执行:测试 A=主仓库社招技术 JD 端到端+样式自定义;
测试 B=只有技能包的空仓库+金融校招;测试 C=污染过的 tracker(未知状态/坏日期/
失效引用/空下一步)。审查点:工作流是否走通、是否仍在手写确定性代码、HITL 是否守住。

### 结果:三条工作流全部走通;HITL 零违例;确认 5 处 scripts 遗漏(均已修复)
1. PNG 预览生成被手写两次 → render_pdf.py 加 --preview
2. 全角标点归一化被手写两遍(第一版静默失败) → 新增 scripts/normalize_punct.py
   (修复记录:第一稿替换字符在文件传输中变成半角,改用 \uFF0C 码点定义后修复;
   幂等性、标签不误伤均有测试)
3. check_facts 对 HTML 注释元数据误报 → 剥注释再提取
4. render_pdf 的 CJK 换行假阴性("8.5\n万")→ 采纳测试 agent A 的空白归一化补丁
5. 投递节奏周统计被心算 → check_tracker.py 扩展周节奏输出;
   顺带修复重构时丢失的表头跳过逻辑(自己的测试自己抓到)

### 结构决策(第一性原理):技能包 = 完整系统层
测试 B 暴露分发单元割裂:skill 在 .agents/skills/,scripts/templates 在仓库根,
导致"只有技能的新仓库"不可用。修正:scripts/ 与 templates/(含 states.yml 母本)
整体收进技能包,技能即完整可移植系统;仓库只留数据层。
SKILL.md 第 0 步写明数据层骨架规格,冷启动不再依赖外部副本。

### 文档缺口修复(全部来自测试报告的摩擦清单)
离线/预授权改写流程、素材过薄时填充率让位事实完整性、star-bank 为空的
评估降级输出、表外状态词的最保守映射默认、不可修复 FAIL 的占位文件策略、
合规修复 vs 篡改历史的边界、周报同名追加规则、红旗满分基准、tagline 与
概述模块的边界、色名无色值时的默认策略、空表合法状态。

## 追加(2026-09-03):全能型扩展 — 权威信源、多领域、校招社招与乐高式自定义

### 1. 简历内容权威标准的界定与伪科学辨析
- **为什么必须设立信源门禁**：网络上充斥着“简历必须有彩色头像框”、“要用炫酷进度条标技能百分比”、“必须写自我评价夸自己抗压”等自媒体水文与商业博客贩卖焦虑的糟粕。这些做法不仅破坏 ATS 机器解析（进度条在文本抽取中变成乱码或空白），还容易引起专业面试官的反感。
- **采纳的权威信源金字塔**：
  1. Google 官方招聘标准（Laszlo Bock 提出并在 Alphabet 全球落地的 Google XYZ 黄金公式）；
  2. 哈佛大学（Harvard Mignone Center / HBS CMC）PAR 结构与动作动词分类全库，杜绝软弱开篇词；
  3. 行业专业协会（SHRM、AICPA/CFA、ACM/IEEE、WSO）的垂直规范；
  4. 顶尖 ATS 系统（Workday, Taleo, Greenhouse, 国内 Moka）的语义化解析安全边界。

### 2. 多领域策略包（Domain Strategy Packs）的论证
- **结论**：**必须严格分领域**。各领域的价值衡量标准存在本质差异：
  - **技术/研发 (Tech)**：看重系统稳定性（SLA）、性能时延（ms）、并发量（QPS）、技术栈关键词显性覆盖。
  - **金融/财务 (Finance)**：看重资产规模（AUM）、收益率（IRR）、合规、财务模型；审美要求极度保守（一页铁律、纯黑白）。
  - **人力资源 (HR)**：看重招聘周期（Time-to-fill）、留存率、人均效能、劳动法规与组织发展。
  - **产品/运营 (Product & Ops)**：看重用户生命周期（LTV/CAC）、关键转化漏斗、留存与 GMV 增量。
  - **销售/市场 (Sales & Marketing)**：看重签约业绩达成率、商机管线（Pipeline）、ROAS。
  - **设计/体验 (Design)**：看重设计系统规范复用度、可用性得分（SUS）、作品集转化。
  - **数据科学 (Data Science)**：看重模型精度（AUC/F1）、归因提升、查询性能优化。
- **架构保证**：领域只影响“量化对象、模块侧重点、术语体系与篇幅约束”，底层的“事实完整性”、“XYZ 公式”、“对齐校验”对所有领域一律生效。

### 3. 校招 vs 社招 vs 实习的差异化分流
- **校招 (Campus)**：考核“潜力与基本盘”。教育背景置顶第一屏（学校、专业、GPA、主修课、奖学金）；实习、课程项目、学术竞赛补位；严格 1 页。
- **社招 (Social)**：考核“即插即用战力与可验证业绩”。工作经历置顶第一屏（倒序，详写近 3~5 年成果）；教育背景后置底部仅占 1~2 行；篇幅 1~2 页。
- **实习 (Internship)**：考核“学习敏捷度与到岗时间保障”。明确每周出勤天数与实习周期。

### 4. 乐高式插槽自定义架构 (Modular Slot Architecture)
- **拒绝“死板模板”与“生硬渲染引擎”的平衡之道**：
  如果用纯代码（如 React/Vue）做渲染引擎，会带来沉重的外部依赖与黑盒复杂度；如果用固定不变的几套静态 HTML，用户想“去掉概述”、“教育放前面”、“加个竞赛奖项”就会寸步难行。
- **破局点**：**声明式配置 Schema（`style.yml`） + 语义化模块积木（Slot Blocks）**。
  - 用户用自然语言表达：“墨绿色、标题居中、教育放最前面、不要概述、技能用胶囊样式”。
  - AI 代理直接映射至 Schema：
    - `theme_palette: emerald-tech`（或映射 Hex 写入 CSS 变量 `--accent`）
    - `header.align: center`（Header 应用对应样式）
    - `sections: [education, work, projects, skills, awards]`（按列表顺序组装 HTML 块，未列出的自动不渲染）
    - `skills_style: pills`（技能项渲染为带有轻底色的优雅圆角胶囊）
  - 交付前全部通过 `render_pdf.py` 自动化验证（页数、ATS 文本可抽取性、版面填充率）。
